...
export default function FormProduct({ setOpen, setAlerta, product }) {
    ...
    if (product) {
      updateProduct(data, product.id)
        .then(() => {
         ...
         setTimeout(()=> {Router.push('/dashboard/products')},5000);
        })
        .catch((error) => {
          setAlert({
            active: true,
            message: `Ha habido un error: ${error.message}`,
            type: 'error',
            autoClose: false,
          });
        });
            ...
            <div className="col-span-6">
              ...
              >
                {categoriesList.map((category, index) => (
                  <option value={`${category.id}`} selected={category.id==product?.category?.id} key={`option-${index}`}>
                    {category.name}
                  </option>
                ))}
              </select>
